...
class Table extends EmitterComponent {
    ...
    #searchListener() {
        ...
        if (this.searchInput) {
            this.searchInput.addEventListener('input', (event) => debouncedHandleSearch(event.target.value));
        }
    }
            ...
            exportItem.addEventListener('click', (event) => {
                ...
                setTimeout(() => {
                    ...
                    $(`#${this.tableId}`).tableExport({
                        ...
                        onCellData: ($cell, row, col, href, cellText, cellType) => {
    ...
    #createPageItem(pageNumber, maxPagesToShow = 5) {
        ...
        pageLink.addEventListener('click', (event) => {
            event.preventDefault();
            const currentActive = this.container.querySelector(`#${this.tableId}-pagination-data .page-item.active`);
            const currentActivePage = currentActive ? parseInt(currentActive.querySelector('a').dataset.page) : null;
            ...
            if (pageNumber == "previous") {
                ...
                currentPageNumber = currentActivePage < this.totalPages ? currentActivePage + 1 : this.totalPages;
            } if (pageNumber == "start") {
                currentPageNumber = 1
            } else if (pageNumber == "end") {
