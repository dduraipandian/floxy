...
class Dropdown extends EmitterComponent {
        ...
        if (element) {
            if (show) {
                ...
                if (transition) {
                    ...
                    setTimeout(() => { element.style.opacity = '100'; }, 500);
                } else {
    ...
    init() {
        ...
        this.dropDownItemSearchContainer = this.element.querySelector(`#${this.dropDownItemSearchContainerId}`);
    ...
    #animateDropDown() {
        ...
        dropDown.addEventListener('hide.bs.dropdown', event => this.displayAnimatedItems(false));
    }
