    ...
    registerElement(element) {
        ...
        if (typeof window !== 'undefined' && window.formElements && element.getFormId) {
            ...
            if(window.formElements[formKey][element.id]) {
                ...
                console.warn(`Element with id ${element.id} is already registered to form ${formId}. overwriting it.`);
            }
...
class BasePrimitiveElement extends BaseElement {
    ...
    #getElementDom() {
        ...
        if (!this.#dom) {
            throw new Error(`DOM element with id ${this.id} not found. Element should be rendered first.`);
        }
