...
class ArrayGroup extends BaseNonPrimitiveElement {
    ...
    addArrayGroup() {
        ...
        let group = new Group({
            id: `ag-${groupId}`, name: groupId || "Group",
    ...
    removeGroup(groupId) {
        try {
            ...
            if (groupElement) {
                ...
                const groupIndex = this.formElements.findIndex(element =>
                    element.id === `ag-${groupId}`
                );
    ...
    hasValidPrimaryKeys(value) {
        ...
        return this.options['primary-keys'].every(key => value[key] !== undefined && value[key] !== null && value[key] !== '');
    }
}
