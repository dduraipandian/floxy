...
class ThemeEditor extends EmitterComponent {
  ...
  html() {
    return `      
            <button class="floxy-toggle-btn" id="floxy-theme-toggle">
            ...
            <div class="floxy-theme-editor" id="floxy-theme-panel">
                <div class="floxy-theme-header">
                    <h5 class="m-0 fw-bold" style="font-size: 1rem">Floxy Theme Editor</h5>
                    <button type="button" class="btn-close" id="floxy-theme-close" style="font-size: 0.75rem"></button>
                </div>
                <div class="floxy-theme-body">
                    ${this.renderTokenControl()}
                </div>
                <div class="floxy-theme-footer">
                    <button class="btn btn-sm btn-outline-primary mb-2" style="width: 140px;" id="floxy-export-css">Copy CSS Variables</button>
                    <button class="btn btn-sm btn-outline-success mb-2" style="width: 140px;" id="floxy-download-theme">Download Theme (.css)</button>
                    <button class="btn btn-sm btn-outline-secondary w-100" id="floxy-theme-reset">Reset to Default</button>
                </div>
            </div>
        `;
  }
  ...
  #getCategory(section) {
    ...
    let html = `<h6 class="m-0 fw-bold">${section.category}</h6>`;
    if (group) {
      html += '<div style="display: flex; gap: 10px; flex-wrap: wrap;">';
      section.tokens.forEach((token) => {
        const currentValue = this.manager.getToken(token.name);
        html += `<input type="color" title="${token.label}" class="floxy-color-input group" data-token="${token.name}" value="${this.rgbToHex(currentValue)}">`;
      });
      html += "</div>";
      return html;
    }
  ...
  #getColorTokenControl(token) {
    // If it's a hex or named color, great. If it's a var(), we might need more logic but let's assume raw values for now.
    const currentValue = this.manager.getToken(token.name);
    return `
        <div class="floxy-token-row">
          <span class="floxy-token-label">${token.label}</span>
          <div class="floxy-token-control">
  ...
  #getRangeTokenControl(token) {
    ...
    return `
        <div class="floxy-token-row" style="align-items: flex-start; flex-direction: column;">
          <div style="display: flex; justify-content: space-between; width: 100%; margin-bottom: 4px;">
            <span class="floxy-token-label">${token.label}</span>
            <span class="floxy-range-value" id="val-${token.name}">${currentValue}</span>
          </div>
          <input type="range" class="form-range floxy-range-input w-100" 
  ...
  attachEvents() {
    ...
    panel.querySelectorAll("input").forEach((input) => {
      input.oninput = (e) => {
        ...
        const valDisplay = document.getElementById(`val-${token}`);
    ...
    exportBtn.onclick = () => {
      ...
      navigator.clipboard.writeText(css).then(() => {
        ...
        exportBtn.classList.replace("btn-outline-primary", "btn-success");
        setTimeout(() => {
          exportBtn.textContent = originalText;
          exportBtn.classList.replace("btn-success", "btn-outline-primary");
        }, 2000);
      });
    };
