    ...
    return {
        EmitterComponent: class {
            constructor(options) {
                ...
                // Defer document access to instantiation time (which happens in test)
                // But wait, this class IS instantiated in test. 
                // However, the class definition is returned by factory.
                // To be safe, we won't set this.container here if it causes issues, 
                // but usually it's fine if we don't use the global directly in the factory scope?
                // The error was "Invalid variable access: document". 
                // So we can't write `document.createElement` here.
...
class ConcreteNodeView extends BaseNodeView {
    getNodeElement() { return "<div>Node Content</div>"; }
}
    ...
    test("should calculate port position correctly", () => {
        ...
        const mockPort = {
            getBoundingClientRect: jest.fn(() => ({
                left: 290, top: 120, width: 10, height: 10
            }))
        };
